...
import models.fault.Process_Task;
import web.Resource;
...
@Path("api/Process_Task")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class Process_TaskResourses extends Resource<Process_Task> {
    ...
    @POST
    @Transactional
    public Response create(Process_Task m) {
    ...
    @Transactional
    @Path("{id}")
    public Response update(@PathParam int id, Process_Task model) {

        Process_Task process_task = Process_Task.findById(id);
        if (process_task == null) {
            throw new WebApplicationException("RCA Item with this Id doesn't exsist! ", 404);
        }
        // rca.rca_id = model.rca_id;
        process_task.name = model.name;
        process_task.description = model.description;
        process_task.related_TaskApp = model.related_TaskApp;
        process_task.persist();
        return Response.ok(process_task).build();
    }
